...
// Fetch exam schedule for the student's class
$stmt = $conn->prepare("
    SELECT exam_name, semester, exam_date 
    FROM exams 
    WHERE class = ? AND exam_date >= CURDATE()
    ORDER BY exam_date
");
if (!$stmt) {
    die("Database prepare error.");
...
<head>
    ...
    <title>Student Dashboard - Exam Schedule</title>
